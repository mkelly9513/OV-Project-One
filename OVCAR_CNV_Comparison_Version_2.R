...
plot(density(as.numeric(tCNVnolabs)))
hist(as.numeric(tCNVnolabs),breaks = 50)
#this is a normal-ish distribution so a t test should work but just to be sure we can use the the mann whitney U aka the Wilcox test 
#as it does not assume normality 
Wtest<-wilcox.test(as.numeric(tCNVnolabs),as.numeric(tCNVRnnolabs), alternative = 'greater')
...
#adding mann whitney u to test non-parametric 
whitpvals <- c(Wtest$p.value)
...
for (i in c(1:100))
{
  ...
  #unpaired one sided t.test looking for amplification of the SE regions vs randomly drawn set
  ttestres<-t.test(as.numeric(tCNVnolabs),as.numeric(tCNVRnnolabs), paired = FALSE, alternative = 'greater')
  Wtest<-wilcox.test(as.numeric(tCNVnolabs),as.numeric(tCNVRnnolabs), paired = FALSE, alternative = 'greater')
  
  #fill arrays 
  ttestpvals<- c(ttestpvals,ttestres$p.value)
  whitpvals<- c(whitpvals,Wtest$p.value)
  #unpaired estimates 
...
#full genome direct comparison 
tCNVWG <- t(CNV_fullset)
tCNVWGnolabs<- tCNVWG[6:572,]

ttestres<-t.test(as.numeric(tCNVnolabs),as.numeric(tCNVWGnolabs), paired = FALSE, alternative = 'greater')
Wtest<-wilcox.test(as.numeric(tCNVnolabs),as.numeric(tCNVWGnolabs), paired = FALSE, alternative = 'greater')
